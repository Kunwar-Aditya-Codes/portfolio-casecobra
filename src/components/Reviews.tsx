...
const ReviewColumn = ({
  ...
  const coloumnRef = useRef<HTMLDivElement | null>(null);
  const [coloumnHeight, setColoumnHeight] = useState<number>(0);
  const duration = `${coloumnHeight * msPerPixel}ms`;

  useEffect(() => {
    if (!coloumnRef.current) return;

    const resizeObserver = new window.ResizeObserver(() => {
      setColoumnHeight(coloumnRef.current?.offsetHeight ?? 0);
    });

    resizeObserver.observe(coloumnRef.current);
